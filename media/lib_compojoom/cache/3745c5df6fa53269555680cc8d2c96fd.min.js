 ...
 */
(function($){$(document).ready(function(){$('*[rel=tooltip]').tooltip();$('.hasTooltip').tooltip({"html":true,"container":"body"});})})(jQuery);
...
DPGlobal.headTemplate+
DPGlobal.contTemplate+'</table>'+'</div>'+'</div>';}(window.jQuery);(function($){$(document).ready(function(){$(window).load(function(){$("#loading").fadeOut("slow");})
$('.slimscroller').slimscroll({height:'auto',size:'3px',railOpacity:0.3,wheelStep:5});$('.chat-widget').slimScroll({height:'300px',color:'#868686',size:'3px',railOpacity:0.3,wheelStep:5});$('.scroll-widget').slimScroll({height:'325px',position:'left',color:'#868686',size:'3px',railOpacity:0.3,wheelStep:5});$('.scroll-user-widget').slimScroll({height:'500px',color:'#868686',size:'3px',railOpacity:0.3,wheelStep:5});$('.dropdown-message-scroll').slimScroll({height:'250px',position:'left',color:'#868686',size:'3px',railOpacity:0.3,wheelStep:5});$('.table-scroll').slimScroll({height:'300px',position:'right',color:'#868686',size:'3px',railOpacity:0.3,wheelStep:5});$("button.show-sidebar").click(function(){$("div.left").toggleClass("mobile-sidebar");$("div.right").toggleClass("mobile-content");$("div.logo-brand").toggleClass("logo-brand-toggle");});$('#sidebar-menu > ul > li > a').click(function(){$('#sidebar-menu li').removeClass('selected');$(this).closest('li').addClass('selected');var checkElement=$(this).next();if((checkElement.is('ul'))&&(checkElement.is(':visible'))){$(this).closest('li').removeClass('selected');checkElement.slideUp('fast');}
if((checkElement.is('ul'))&&(!checkElement.is(':visible'))){$('#sidebar-menu ul ul:visible').slideUp('fast');checkElement.slideDown('fast');}
if($(this).closest('li').find('ul').children().length==0){return true;}else{return false;}});$('.datepicker-input').datepicker();var hash=window.location.hash.substring(1);if(hash.length){$('[key='+hash+']').addClass('active');}
$('[key]').click(function(){$('[key]').removeClass('active');var hash=$(this).attr('key');$('[key='+hash+']').addClass('active');});$('#csearch').keyup(function(){var sw=$(this).val();if(sw.length>1){$('#sidebar-menu li').hide();$('#sidebar-menu li[keywords*="'+sw+'"]').show();$('#sidebar-menu li[keywords*="'+sw+'"]').parents('li').show();}else{$('#sidebar-menu li').show();}});$('#c-system-message-container').html($("#system-message-container").html());$("#system-message-container").on('DOMNodeInserted DOMNodeRemoved',function(){$('#c-system-message-container').html($("#system-message-container").html());});$('#c-system-message-container').html($('.c-debug-msg').html());$('#c-system-message-container').append($("#system-debug").html());var jdoc=$(document);var refreshDocHeight=function(){$('.left').css('min-height',jdoc.height()-30);};window.setInterval(refreshDocHeight,500);$('.left ').click(function(){toggleSideMenu();}).children().click(function(e){return false;});$('#sidebar-menu a').click(function(){window.location.href=$(this).attr('href');return true;})});})(jQuery);function toggleSideMenu(){jQuery('.left').toggleClass('side-menu-collapse');jQuery('.right').toggleClass('content-page-menu-collapse');}
...
this.listen()
this.checkVal()}
Inputmask.DEFAULS={mask:"",placeholder:"_",definitions:{'9':"[0-9]",'a':"[A-Za-z]",'?':"[A-Za-z0-9]",'*':"."}}
