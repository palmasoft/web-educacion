    ...
    */
    public static function getMenuItemids($catid = 0){
        ...
        if (!$jdItemids['root'] && $catid == 0){
            $sql = 'SELECT id FROM #__menu WHERE (link = ' . $db->Quote('index.php?option=com_jdownloads&view=downloads'). ') AND published = 1 AND client_id = 0 AND language = '.$db->Quote($active_language).' AND access IN ('.$access_levels.')' ;
            $db->setQuery($sql);
            $jdItemids['root'] = $db->loadResult();
            if (!$jdItemids['root']){
                $sql = 'SELECT id FROM #__menu WHERE (link = ' . $db->Quote('index.php?option=com_jdownloads&view=downloads'). ') AND published = 1 AND client_id = 0 AND access IN ('.$access_levels.')' ;
                $db->setQuery($sql);
                $jdItemids['root'] = $db->loadResult();
            }
        }
        ...
        $sql = 'SELECT id FROM #__menu WHERE link = ' . $db->Quote('index.php?option=com_jdownloads&view=search'). ' AND published = 1 AND client_id = 0 AND language = '.$db->Quote($active_language).' AND access IN ('.$access_levels.')' ;
        $db->setQuery($sql);
        ...
        $sql = 'SELECT id FROM #__menu WHERE link = ' . $db->Quote('index.php?option=com_jdownloads&view=form&layout=edit'). ' AND published = 1 AND client_id = 0 AND language = '.$db->Quote($active_language).' AND access IN ('.$access_levels.')' ;
        $db->setQuery($sql);
    ...
    */
    public static function getHTML5Player($file, $media_path)
    {
        ...
        if ($playertype == 'video'){
            ...
            $player_maxwidth = 'max-width:'.(int)$jlistConfig['html5player.audio.width'].'px; ';
    ...
    public static function existsCustomFieldsTitles()
    {
        ...
        for ($i=1; $i<15; $i++){
            if ($jlistConfig["custom.field.$i.title"] != ''){
               $custom_array[] = $i;
               $custom_titles[] = self::getOnlyLanguageSubstring($jlistConfig["custom.field.$i.title"]);
               if(isset($jlistConfig["custom.field.$i.values"])) {
                  $custom_values[] = explode(',', self::getOnlyLanguageSubstring($jlistConfig["custom.field.$i.values"]));   
    ...
    public static function getUserLimits($user_rules, $marked_files_id){
        ...
        */
        ...
        if (!isset($consumed['today'])){
            $consumed['today'] = new StdClass;
            $consumed['today']->sumsize = 0;
            $consumed['today']->sumfiles = 0;
        }
        ...
        */
        ...
        if (!isset($consumed['week'])){
            $consumed['week'] = new StdClass;
            $consumed['week']->sumsize = 0;
            $consumed['week']->sumfiles = 0;
        }
        ...
        */
        ...
        if (!isset($consumed['month'])){
            $consumed['month'] = new StdClass;
            $consumed['month']->sumsize = 0;
            $consumed['month']->sumfiles = 0;
        }
        ...
        if (file_exists($api_AUP)){
            ...
            if ($profil){
                ...
                if ($jlistConfig['user.can.download.file.when.zero.points']){
                        ...
                        $aup_result['points_info']  = sprintf( str_replace('%d', '%s', JText::_('COM_JDOWNLOADS_FE_VIEW_AUP_SUM_POINTS')), self::strToNumber($sum_aup_points,'','','2'), self::strToNumber($profil->points));
        ...
        if ($jlistConfig['use.alphauserpoints'] && $user_id){
            ...
            if (file_exists($api_AUP)){
                ...
                if (file_exists($api_AUP)){
                    ...
                    if ($referreid){
                        $key_reference = AltaUserPointsHelper::buildKeyreference( 'plgaup_jdownloads_user_download_use_price', $file_id, $user_id );
            ...
            if (file_exists($api_AUP)){
                ...
                foreach ($files as $file){  
                    if ($file->submitted_by){
                        ...
                        if ($referreid){
                            $key_reference = AltaUserPointsHelper::buildKeyreference( 'plgaup_jdownloads_downloader_to_uploader', $file->file_id, (int)$file->submitted_by );
            ...
            if (file_exists($api_AUP)){
                ...
                foreach ($files as $file){  
                    if ($file->submitted_by){
                        $referreid = AltaUserPointsHelper::getAnyUserReferreID( (int)$file->submitted_by );
                        if ($referreid){
                            $key_reference = AltaUserPointsHelper::buildKeyreference( 'plgaup_jdownloads_downloader_to_uploader_use_price', $file->file_id, (int)$file->submitted_by );
