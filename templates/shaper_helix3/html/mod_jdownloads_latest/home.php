...
<style type="text/css">
    ...
    .archivo_para_descargar .titulo {
         width: <?=(int)$view_thumbnails_size?>px;
         white-space: nowrap;
         overflow: hidden;
    ...
    .archivo_para_descargar:hover,
    .archivo_para_descargar:focus {
...
<?php
    ...
    if ($files){
        ...
        for ($i=0; $i<count($files); $i++) {
            $html .= '<div title="'.( $files[$i]->file_title.' '.$version.$files[$i]->release ).'" class="archivo_para_descargar" style="float:left; padding: 3px 6px ; margin: 0px 2px 4px 2px;">';   
			...
			if ($view_pics){
				...
				$files_pic = '<a href="'.$link.'">'.'<img src="'.JURI::base().'images/jdownloads/fileimages/'.$files[$i]->file_pic.'" width="'.$size.'" height="'.$size.'" style="border: 0px; vertical-align: top;margin: auto;"'.' alt="" /> '.'</a>';
			...
			$html .= '<div class="titulo" style="padding-bottom: 3px; text-align: '.$alignment.';">'.$number.$files_pic.$link_text.'</div>';
            ...
            if ($view_thumbnails){
                ...
                if ($thumbnail) $html .= '<a href="'.$link.'">'.'<div style="padding-bottom: 3px; text-align:'.$alignment.'">'.$thumbnail.'</div>'.'</a>';
            }
