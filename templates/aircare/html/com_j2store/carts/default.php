						...
						<div class="j2store-cart-buttons">
							...
							<span class="cart-continue-shopping-button">
								<?php if($this->continue_shopping_url->type != 'previous'): ?>
									<input class="btn btn-success" type="button" onclick="window.location='<?php echo $this->continue_shopping_url->url; ?>';" value="<?php echo JText::_('J2STORE_CART_CONTINUE_SHOPPING'); ?>" />
								<?php else: ?>
							...
							<span class="cart-update-button">
								<input class="btn btn-primary" type="submit" value="<?php echo JText::_('J2STORE_CART_UPDATE'); ?>" />
							</span>
							</div>
							<div class="buttons-right">
								<?php if($this->params->get('show_clear_cart_button', 0)):?>
								<span class="cart-clear-button">
									<a class="btn btn-danger" href="<?php echo JRoute::_('index.php?option=com_j2store&view=carts&task=clearCart');?>"><?php echo JText::_('J2STORE_EMPTY_CART')?></a>
								</span>
								<?php endif;?>
							</div>
						</div>
